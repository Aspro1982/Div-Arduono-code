...
int TCPClient_Close()
{
    ...
    // send AT-command to ESP8266 to close the connection
    esp8266.println("AT+CIPCLOSE");

    
    // waits until ESP8266 send back "OK" (success = true) or "ERROR" (success = false)
...
int TCPClient_Connect(String IPAddressString, int Port)
{
    int Result = 0;

    esp8266.setTimeout(5000);
    
    // send AT-command to ESP8266 to connect to TCP/IP-server
    esp8266.println("AT+CIPSTART=\"TCP\",\"" + IPAddressString + "\"," + String(Port) + "\r\n");
...
{
    int TCPBytesReceivedCount = 0;
    
    byte ESPBytesReceivedBuffer[128];

    // get the string the ESP8266 sends back when something was received from server
    // string begins with +IPD
    if (esp8266.find("+IPD"))
    {  
        // read until return character
        byte ESPBytesReceivedCount = esp8266.readBytesUntil('\r', ESPBytesReceivedBuffer, 120);

        // remove everything before the ":" character and copy the characters sent back from server in TCPBytesReceivedBuffer
        int c = 0;
        while (ESPBytesReceivedBuffer[c] != 58)
        {
          c++;
        }
        c++;
        ...
        while (c < ESPBytesReceivedCount)
        {
          TCPBytesReceivedBuffer[c - pos] = ESPBytesReceivedBuffer[c];
          c++;
        }
        TCPBytesReceivedCount = c - pos;         
    }
...
int TCPClient_Send(byte data[], int len)
{
     ...
     String Send = "AT+CIPSEND=" + String(len) + "\r\n";
     ...
     if (success == true)
     {
        esp8266.write(data,len);

        
        // waits until ESP8266 send back "OK" (success = true) or "ERROR" (success = false)
        success = esp8266.findUntil("OK", "ERROR");        
...
String txtSSID = "YOUR SSID";
String txtPassword = "YOUR PASSWORD";


#define PIN_LED_BOARD  13
#define PIN_ACTOR      13   // the digital output we want to control
...
void setup()
{
  ...
  // send AT-command to ESP8266 to reset the chip
  
  esp8266.setTimeout(5000);
  
  esp8266.println("AT+RST");

  // waits until ESP8266 sends back "OK" (success = true) or "ERROR" (success = false)
  success &= esp8266.findUntil("OK", "ERROR");

  
  // send AT-command to ESP8266 to put ESP8266 in client mode
  ...
  delay(2000);   // we need this delay to be sure that we react on the right "OK"

 
  
  // send AT-command to ESP8266 to log on local WiFi network
  
  esp8266.setTimeout(10000);
  
  esp8266.println ("AT+CWJAP=\"" + String(txtSSID) + "\",\"" + String(txtPassword) + "\"");
